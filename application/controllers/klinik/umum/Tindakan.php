...
class Tindakan extends CI_Controller{
	public function index()
	{
		...
		$data['tindakan'] = $this->db->query("SELECT * FROM pasien ps, tindakan_umum tu WHERE ps.nomor_pasien=tu.nomor_pasien AND ps.status ='Umum' ORDER BY id_tindakan DESC")->result();
		$this->load->view('templates_klinik/header');
		$this->load->view('templates_klinik/sidebar');
		$this->load->view('klinik/umum/tindakan/tindakan', $data);
	...
	public function data_pasien()
	{
		...
		$this->load->view('templates_klinik/header');
		$this->load->view('templates_klinik/sidebar');
		$this->load->view('klinik/umum/tindakan/data_pasien', $data);
		$this->load->view('templates_klinik/footer');
	}
	public function detail_tindakan($id)
	{
		$data['tindakan'] = $this->db->query("SELECT * FROM pasien ps, tindakan_umum tu WHERE ps.nomor_pasien=tu.nomor_pasien AND tu.id_tindakan='$id' ORDER BY id_tindakan")->result();
		$this->load->view('templates_klinik/header');
		$this->load->view('templates_klinik/sidebar');
		$this->load->view('klinik/umum/tindakan/detail_tindakan', $data);
		$this->load->view('templates_klinik/footer');
	}
	public function detail_data_pasien($id)
	{
		...
		$this->load->view('templates_klinik/header');
		$this->load->view('templates_klinik/sidebar');
		$this->load->view('klinik/umum/tindakan/detail_pasien', $data);
		$this->load->view('templates_klinik/footer');
	}
	public function print_invoice($id)
	{	
		$data['tindakan'] = $this->db->query("SELECT * FROM pasien ps, tindakan_umum tu WHERE ps.nomor_pasien=tu.nomor_pasien AND tu.id_tindakan='$id' AND tu.kategori_pasien ='Umum' ORDER BY id_tindakan")->result();
		$this->load->view('klinik/umum/tindakan/print_invoice', $data);
	}
	public function form_tindakan_umum($id)
	{
		date_default_timezone_set('Asia/Jakarta');
		$data['pasien'] = $this->Models_klinik->ambil_id_pasien($id);
		$this->load->view('templates_klinik/header');
		$this->load->view('templates_klinik/sidebar');
		$this->load->view('klinik/umum/tindakan/form_tindakan', $data);
		$this->load->view('templates_klinik/footer');
	}
	public function form_tindakan_umum_aksi()
	{
		$this->_rules();
		if($this->form_validation->run() == FALSE){
			$this->form_tindakan_umum();
		}else{
			...
			$this->Models_klinik->insert_data('tindakan_umum', $data);
			$this->session->set_flashdata('pesan',
										  '<div class="alert alert-success alert-dismissible show fade">
								                <div class="alert-body">
								                    ...
								                    Tindakan Umum Berhasil Ditambahkan.
								                </div>
							                </div>');
			redirect('klinik/umum/tindakan');
		}
	}
